                              ...
                              NrRrcSap::MeasurementReport meas)
{
    NS_LOG_UNCOND(Simulator::Now().GetSeconds()
                  ...
                  << " - Report from CellId " << cellId);
    NS_LOG_UNCOND("  rsrp = " << (int)meas.measResults.measResultPCell.rsrpResult << " ("
                              << (int)meas.measResults.measResultPCell.rsrpResult - 140
                              << " dBm) | rsrq = "
                              << (int)meas.measResults.measResultPCell.rsrqResult << " ("
                              << ((int)meas.measResults.measResultPCell.rsrqResult - 40) / 2.0
                              << " dB)");
    for (const auto& neighbor : meas.measResults.measResultListEutra)
    {
        NS_LOG_UNCOND("  Neighbor PCI " << neighbor.physCellId
                                        << ": rsrp = " << (int)neighbor.rsrpResult << " ("
                                        << (int)neighbor.rsrpResult - 140
                                        << " dBm) | rsrq = " << (int)neighbor.rsrqResult << " ("
                                        << ((int)neighbor.rsrqResult - 40) / 2.0 << " dB)");
    }
}
...
PrintUePosition(NetDeviceContainer ueNetDevs, NodeContainer ueNodes)
{
    for (uint32_t ueId = 0; ueId < ueNodes.GetN(); ++ueId)
    {
        ...
        NS_LOG_UNCOND(Simulator::Now().GetSeconds()
                      << "s - UE " << ueId << " position: " << uepos);
    }

    Simulator::Schedule(MilliSeconds(500), &PrintUePosition, ueNetDevs, ueNodes);
}
...
ChooseCalibrationScenario(Parameters& params)
{
    if (params.confType == "calibrationConf")
    {
        ...
        if (params.radioNetwork == "NR")
        {
            ...
            params.ueTxPower = 23;
            params.speed = 3; // in m/s (10.8 km/h)
...
{
    // Set a non-zero speed for mobility by default
    if (params.speed == 0)
    {
        params.speed = 15.0; // 54 km/h
    }
    ...
    Config::SetDefault("ns3::NrRlcUm::MaxTxBufferSize", UintegerValue(99999g));
    ...
    if (nrHelper != nullptr)
    {
        ...
        handoverAlgo->SetAttribute("NeighbourCellOffset",
                                   IntegerValue(2)); // 1dB
        nrHelper->SetHandoverAlgorithm(handoverAlgo);
        ...
        reportConfigA4.triggerType = NrRrcSap::ReportConfigEutra::EVENT;
        ...
        reportConfigA4.threshold1.range = 33; // RSRQ range is 0-34. 33 corresponds to -3.5dB
        reportConfigA4.triggerQuantity = NrRrcSap::ReportConfigEutra::RSRQ;
        reportConfigA4.reportQuantity = NrRrcSap::ReportConfigEutra::BOTH;
        reportConfigA4.reportInterval = NrRrcSap::ReportConfigEutra::MS480;

        for (auto it = gnbNodes.Begin(); it != gnbNodes.End(); ++it)
        {
            Ptr<NrGnbRrc> gnbRrc =
                (*it)->GetDevice(0)->GetObject<NrGnbNetDevice>()->GetRrc();
            gnbRrc->AddUeMeasReportConfig(reportConfigA2);
            gnbRrc->AddUeMeasReportConfig(reportConfigA4);

            std::string gnbPath = "/NodeList/" + std::to_string((*it)->GetId()) +
                                  "/DeviceList/0/NrGnbRrc/RecvMeasurementReport";
            Config::Connect(gnbPath, MakeCallback(&RecvMeasurementReportCallback));
        }

        for (auto it = ueNodes.Begin(); it != ueNodes.End(); ++it)
        {
            std::string uePath = "/NodeList/" + std::to_string((*it)->GetId()) +
                                 "/DeviceList/0/NrUeRrc/HandoverStart";
            Config::Connect(uePath, MakeCallback(&HandoverStartCallback));
            uePath = "/NodeList/" + std::to_string((*it)->GetId()) +
                     "/DeviceList/0/NrUeRrc/HandoverEndOk";
            Config::Connect(uePath, MakeCallback(&HandoverEndOkCallback));
        }
    }
