    ...
    else if (mobilityModel == "waypoint")
    {
        ...
        else
        {
            ...
            if (builtinPath == "hexagonal")
            {
                ...
                NS_LOG_UNCOND("Waypoint mobility: hexagonal path visiting all 7 sites (total time=" << t << "s)");
            }
            ...
            else if (builtinPath == "zigzag")
            {
                ...
                NS_LOG_UNCOND("Waypoint mobility: zigzag path crossing sites (total time=" << t << "s)");
            }
            else if (builtinPath == "highway")
            {
                double highwaySpeed = 20.0;  // m/s (72 km/h)
                Vector start(0, -400, 1.5);  // 100m from Site 5 (south)
                Vector end(0, 400, 1.5);     // 100m from Site 2 (north)
                double distance = (end - start).GetLength();  // ~800m
            ...
            else if (builtinPath == "kaohsiung")
            {
                ...
                struct WaypointData { double x; double y; };
                ...
                double t = 0.0;
                double ueHeight = 1.5;
                waypointMm->AddWaypoint(Waypoint(Seconds(t), Vector(gpxPoints[0].x, gpxPoints[0].y, ueHeight)));
                ...
                for (size_t i = 1; i < gpxPoints.size(); ++i)
                {
                    ...
                    t += dist / ueSpeed;
                    waypointMm->AddWaypoint(Waypoint(Seconds(t), Vector(gpxPoints[i].x, gpxPoints[i].y, ueHeight)));
                }

                NS_LOG_UNCOND("Waypoint mobility: kaohsiung real-world path (speed="
                            << ueSpeed << " m/s, total time=" << t << "s)");
            }
            else
            {
