 ...
 */
NS_LOG_COMPONENT_DEFINE("CttcNrMeasurementReportDemo");
...
{
    NS_LOG_UNCOND("Measurement Report received for IMSI " << imsi);
    NS_LOG_UNCOND("  cellId = " << cellId);
    NS_LOG_UNCOND("  rnti = " << rnti);
    NS_LOG_UNCOND("  rsrp = " << (int)meas.measResults.measResultPCell.rsrpResult);
    NS_LOG_UNCOND("  rsrq = " << (int)meas.measResults.measResultPCell.rsrqResult);
}
...
main(int argc, char* argv[])
{
    ...
    // Connect the callback function to the trace source.
    for (uint32_t i = 0; i < gnbNetDev.GetN(); ++i)
    {
        Ptr<NetDevice> gnbDevice = gnbNetDev.Get(i);
        std::string path = "/NodeList/" + std::to_string(gnbDevice->GetNode()->GetId()) +
                           "/DeviceList/0/NrGnbRrc/RecvMeasurementReport";
        Config::Connect(path, MakeCallback(&RecvMeasurementReportCallback));
    }

    // Create a ReportConfigEutra for event-based reporting
    NrRrcSap::ReportConfigEutra reportConfig;
    // reportConfig.reportConfigId = 1; // Activate the config
    reportConfig.triggerType = NrRrcSap::ReportConfigEutra::EVENT; // Set trigger to EVENT
    reportConfig.eventId = NrRrcSap::ReportConfigEutra::EVENT_A2; // Serving becomes worse than threshold
    reportConfig.threshold1.choice = NrRrcSap::ThresholdEutra::THRESHOLD_RSRP;
    reportConfig.threshold1.range = 40; // Threshold = 40 - 140 = -100 dBm
    reportConfig.triggerQuantity = NrRrcSap::ReportConfigEutra::RSRP;
    reportConfig.reportQuantity = NrRrcSap::ReportConfigEutra::SAME_AS_TRIGGER_QUANTITY;
    reportConfig.hysteresis = 0;

    // reportConfig.reportAmount = 0; // not used with periodical reporting
    // reportConfig.maxReportCells = 1;

    // Set the measurement configuration for each gNB
    for (uint32_t i = 0; i < gnbNetDev.GetN(); ++i)
    {
        Ptr<NetDevice> gnbDevice = gnbNetDev.Get(i);
        Ptr<NrGnbNetDevice> nrGnbNetDevice = DynamicCast<NrGnbNetDevice>(gnbDevice);
        Ptr<NrGnbRrc> gnbRrc = nrGnbNetDevice->GetRrc();
        gnbRrc->AddUeMeasReportConfig(reportConfig);
    }
