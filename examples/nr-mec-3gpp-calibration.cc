...
// Main UE IMSI (to filter out background UE events)
...
WriteSimulationResults(const std::string& outputPath, double simTime, double slaThresholdMs)
{
    ...
    // --- Prediction accuracy analysis (episode-based) ---
    // An episode is a contiguous run of the same non-zero predictedTarget in g_predictionLog.
    struct PredictionEpisode {
        double startS;
        double endS;
        uint16_t servingCell;      // serving cell when episode began
        uint16_t predictedTarget;
        std::string outcome;       // "TP", "FP", "ONGOING"
        double hoStartS;           // -1 if not TP
        double leadTimeMs;         // (hoStartS - startS)*1000, -1 if not TP
    };
    std::vector<PredictionEpisode> episodes;

    // Pass 1: segment prediction log into episodes
    bool lastRecordActive = !g_predictionLog.empty() &&
                            g_predictionLog.back().predictedTarget != 0;
    {
        uint16_t curTarget = 0;
        double epStartS = 0.0;
        uint16_t epServingCell = 0;
        double epEndS = 0.0;

        for (const auto& p : g_predictionLog)
        {
            if (p.predictedTarget != curTarget)
            {
                if (curTarget != 0)
                {
                    episodes.push_back({epStartS, epEndS, epServingCell, curTarget,
                                        "?", -1.0, -1.0});
                }
                curTarget = p.predictedTarget;
                epStartS = p.timestampS;
                epServingCell = p.servingCell;
            }
            if (curTarget != 0)
            {
                epEndS = p.timestampS;
    ...
    // Pass 2: classify each episode as TP, FP, or ONGOING
    for (size_t i = 0; i < episodes.size(); ++i)
    {
        auto& ep = episodes[i];
        bool isLastActive = (i == episodes.size() - 1) && lastRecordActive;

        bool matched = false;
        for (const auto& ho : g_handoverEvents)
        {
            if (ho.success && ho.targetCell == ep.predictedTarget &&
                ho.startTime.GetSeconds() >= ep.startS &&
                ho.startTime.GetSeconds() <= ep.endS + 1.0)
            {
                ep.outcome = "TP";
                ep.hoStartS = ho.startTime.GetSeconds();
                ep.leadTimeMs = (ep.hoStartS - ep.startS) * 1000.0;
                matched = true;
                break;
            }
        }
        if (!matched)
        {
            ep.outcome = isLastActive ? "ONGOING" : "FP";
        }
    }
    ...
    for (const auto& ep : episodes)
    {
        ...
        {
            tpEpisodes++;
            predLeadTotal += ep.leadTimeMs;
            predLeadCount++;
            if (ep.leadTimeMs < predLeadMin) predLeadMin = ep.leadTimeMs;
            if (ep.leadTimeMs > predLeadMax) predLeadMax = ep.leadTimeMs;
        }
        else if (ep.outcome == "FP") fpEpisodes++;
        else ongoingEpisodes++;
    }

    // Missed handovers: successful HOs with no matching TP episode
    int missedHandovers = 0;
    for (const auto& ho : g_handoverEvents)
    {
        ...
        {
            if (ep.outcome == "TP" &&
                ep.predictedTarget == ho.targetCell &&
                ho.startTime.GetSeconds() >= ep.startS &&
                ho.startTime.GetSeconds() <= ep.endS + 1.0)
            {
                covered = true;
        ...
        if (!covered) missedHandovers++;
    }

    int totalEpisodes = tpEpisodes + fpEpisodes + ongoingEpisodes;
    double precisionPct = (tpEpisodes + fpEpisodes) > 0
        ? (100.0 * tpEpisodes / (tpEpisodes + fpEpisodes)) : 0.0;
    double recallPct = (tpEpisodes + missedHandovers) > 0
        ? (100.0 * tpEpisodes / (tpEpisodes + missedHandovers)) : 0.0;
    double avgLeadTimeMs = predLeadCount > 0 ? (predLeadTotal / predLeadCount) : 0.0;

    resultsFile << "\n[PREDICTION_ACCURACY]\n";
    resultsFile << "metric,value\n";
    resultsFile << "total_episodes," << totalEpisodes << "\n";
    resultsFile << "tp_episodes," << tpEpisodes << "\n";
    resultsFile << "fp_episodes," << fpEpisodes << "\n";
    resultsFile << "ongoing_episodes," << ongoingEpisodes << "\n";
    resultsFile << "missed_handovers," << missedHandovers << "\n";
    resultsFile << "precision_percent," << std::fixed << std::setprecision(2) << precisionPct << "\n";
    resultsFile << "recall_percent," << std::fixed << std::setprecision(2) << recallPct << "\n";
    if (predLeadCount > 0)
    {
    ...
    resultsFile << "\n[PREDICTION_EPISODES]\n";
    resultsFile << "episode_start_s,episode_end_s,duration_ms,serving_cell,predicted_target,outcome,ho_start_s,lead_time_ms\n";
    for (const auto& ep : episodes)
    {
        double durationMs = (ep.endS - ep.startS) * 1000.0;
        resultsFile << std::fixed << std::setprecision(3)
                    << ep.startS << ","
                    << ep.endS << ","
                    << std::setprecision(1) << durationMs << ","
                    << ep.servingCell << ","
                    << ep.predictedTarget << ","
                    << ep.outcome << ","
                    << std::setprecision(3) << ep.hoStartS << ","
                    << std::setprecision(1) << ep.leadTimeMs << "\n";
    }
    ...
    std::cout << "    Episodes (TP/FP/Ongoing): "
              << tpEpisodes << " / " << fpEpisodes << " / " << ongoingEpisodes << "\n";
    std::cout << "    Missed Handovers:  " << missedHandovers << "\n";
    std::cout << "    Precision:         " << std::fixed << std::setprecision(1) << precisionPct << "%\n";
    std::cout << "    Recall:            " << std::setprecision(1) << recallPct << "%\n";
    if (predLeadCount > 0)
    {
...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("mobilityStartDelay", "Delay in seconds before UE starts moving (for tap bridge startup)", mobilityStartDelay);
    ...
    if (backgroundUeNodes.GetN() > 0)
    {
        ...
        for (uint32_t i = 0; i < backgroundUeNodes.GetN(); i++)
        {
            ...
            bgSourceApp.Start(Seconds(2.0 + i * 0.01));
        }

        NS_LOG_UNCOND("\nBackground DL traffic: " << backgroundUeNodes.GetN() << " UEs across gNBs ["
